...
//     const { pid, variantId } = req.params; // `pid` là ID sản phẩm, `variantId` là SKU hoặc _id của variant
//     const { title, price, color } = req.body;
//     const files = req?.files;
...
// });
// const updateVariant = asyncHandler(async (req, res) => {
//     const { pid, variantId } = req.params; // Lấy pid và variantId từ URL params
//     const { title, price, color } = req.body;
//     const files = req.files;
...
//     // Cập nhật variant trong sản phẩm
//     const updatedProduct = await Product.findOneAndUpdate(
//         { _id: pid, 'variants._id': variantId }, // Kiểm tra đúng ObjectId của variant
//         {
//             $set: {
//                 'variants.$.title': title,
//                 'variants.$.price': price,
